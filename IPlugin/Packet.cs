...
namespace Sheriff.Security
{
    public class Packet
    {
        ...
        public void Replace(Packet rhs)
        {
            lock (rhs.m_lock)
            {
                m_lock = new object();
        ...
        private String ReadAscii(int codepage)
        {
            lock (m_lock)
            {
                if (!m_locked)
        ...
        public String ReadAscii(int lenght,int codepage)
        {
            lock (m_lock)
            {
                if (!m_locked)
                ...
                byte[] bytes = m_reader.ReadBytes(lenght);
